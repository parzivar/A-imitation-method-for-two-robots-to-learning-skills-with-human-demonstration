...
class AbbRobot:
    def __init__(self, host, robot_port, gripper_port, workspace_limits=None, use_dh_griper=False, use_camera=False) -> None:
    ...
    def read_csv_file(self, filename, start_idx, end_idx):
        ...
        with open(filename, 'r') as csvfile:
            reader = csv.reader(csvfile)
            for row in reader:
                joint_angles.append([round(float(x), 1) for x in row[start_idx:end_idx]])
        print("read data success")
        return joint_angles
